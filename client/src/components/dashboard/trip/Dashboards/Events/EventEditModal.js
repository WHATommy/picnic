...
export const EventEditModal = (props) => {
    ...
    const startTime = new Date(props.event.startDate).toLocaleTimeString('en', { timeStyle: 'short', hour12: false, timeZone: 'UTC' });
    const endDate = new Date(props.event.endDate);
    const endIsoDate = endDate.toISOString().substring(0, 10);
    const endTime = new Date(props.event.endDate).toLocaleTimeString('en', { timeStyle: 'short', hour12: false, timeZone: 'UTC' });
    ...
    const handleEditEvent = async (formValue) => {
        ...
        await dispatch(loadAllContent({tripId: props.tripId, contentType: "event"}));
    ...
    return (
        <>
            <Modal.Header closeButton>
                <Modal.Title>Edit {props.event.name}</Modal.Title>
            </Modal.Header>
            <Formik
