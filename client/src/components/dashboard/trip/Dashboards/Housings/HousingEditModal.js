...
export const HousingEditModal = (props) => {
     // Loading state
    ...
    const startDate = new Date(props.housing.startDate);
    const startIsoDate = startDate.toISOString().substring(0, 10);
    const startTime = new Date(props.housing.startDate).toLocaleTimeString('en', { timeStyle: 'short', hour12: false, timeZone: 'UTC' });
    const endDate = new Date(props.housing.endDate);
    const endIsoDate = endDate.toISOString().substring(0, 10);
    const endTime = new Date(props.housing.endDate).toLocaleTimeString('en', { timeStyle: 'short', hour12: false, timeZone: 'UTC' });

    const initialValues = {
        _id: props.housing._id,
        name: props.housing.name,
        location: props.housing.location,
        cost: props.housing.cost,
        website: props.housing.website,
    ...
    const handleEditHousing = async (formValue) => {
        ...
        await dispatch(loadAllContent({tripId: props.tripId, contentType: "housing"}));
    ...
    return (
        <>
            <Modal.Header closeButton>
                <Modal.Title>Edit {props.housing.name}</Modal.Title>
            </Modal.Header>
            <Formik
            ...
            >
                <Form className="p-3"> 
                    <div className="mb-2">
                        ...
                        onChange={(housing) => {
                            let file = housing.currentTarget.files[0];
